...
levels = localStorage.getItem("bollyLevels");
...
function Levelfunct() {
	levels++;
	localStorage.setItem("bollyLevels", levels);
}
...
function Retryminus() {
	if (coins >= 1) {
		...
		document.getElementById("bollyCoins").innerHTML = coins;
	}
}
...
$(document).ready(function () {
	...
	$('.skip').click(function () {
		if (coins < 10) {
			...
			typedAns = '';
			typedAnsNumbers = [];
			hint = 0; localStorage.setItem("bollyHint", hint);
...
function AppenAll() {
	...
	if (localStorage.getItem("bollyHint") == null) {
		hint = 0; localStorage.setItem("bollyHint", hint);
	} else {
		hint = parseInt(localStorage.getItem("bollyHint"));
	}
...
$('.show-hint').click(function () {
	...
	if (coins < 1) {
		...
		HintPlay();
		hint++; localStorage.setItem("bollyHint", hint);
